...
export function SiteProvider({ children }) {
  ...
  async function fetchSiteData() {
    try {
      setIsLoaded(false);
      const [headerResponse, slidesResponse, aboutResponse, specialtiesResponse] = await Promise.all([
        fetch('https://api-nutricare-1.onrender.com/api/header'),
        fetch('https://api-nutricare-1.onrender.com/api/slides'),
        fetch('https://api-nutricare-1.onrender.com/api/about'),
        fetch('https://api-nutricare-1.onrender.com/api/specialties')
      ]);

      if (!headerResponse.ok || !slidesResponse.ok || !aboutResponse.ok || !specialtiesResponse.ok) {
        throw new Error('Falha ao carregar dados do site');
      }

      const [headerData, slidesData, aboutData, specialtiesData] = await Promise.all([
        headerResponse.json(),
        slidesResponse.json(),
        aboutResponse.json(),
  ...
  async function login(username, password, rememberMe) {
    try {
      ...
      if (rememberMe) {
        ...
        sessionStorage.setItem('nutricare_token', authToken);
      }
  ...
  async function updateSpecialtiesData(newSpecialtiesData) {
    console.log(newSpecialtiesData);
    ...
    try {
      ...
      const response = await fetch('https://api-nutricare-1.onrender.com/api/specialties', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
        },
        body: JSON.stringify(newSpecialtiesData),
      });

   console.log(response)
      // Se a resposta for 201 (Created), não tentamos fazer o parse do JSON
      if (response.status === 201) {
        setSiteData(prevData => ({
          ...prevData,
          specialties: newSpecialtiesData,
        }));
        return { success: true, message: 'Especialidades atualizadas com sucesso!' };
      }

      // Se a resposta não for 201, tentamos fazer o parse do JSON (caso a API mude no futuro)
      const updatedSpecialtiesData = await response.json();
      setSiteData(prevData => ({
        ...prevData,
        specialties: updatedSpecialtiesData,
      }));

      return { success: true, message: 'Especialidades atualizadas com sucesso!' };
    } catch (err) {
      console.error('Erro ao atualizar dados das especialidades:', err);
      setError(err.message);
      return { success: false, message: err.message };
    } finally {
      setIsLoaded(true);
